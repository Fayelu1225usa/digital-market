 ...
 * @author faye
 */
public class CustomerCartScene extends javax.swing.JPanel {
    ...
    private ArrayList<OrderBundle> bundles;

    /**
     * Creates new form MarketingDashboard_new
     */
    public CustomerCartScene(JFrame jf, Data data, Account account) {
        setBackground(new Color(0, 128, 128));
        this.jf = jf;
        this.data = data;
        this.account = account; 
        this.cp = data.getBusiness().cd.findCustomer(account.getPerson().getPersonId()); 
        bundles = this.cp.getAccount().getCart().getBundles(); 
        initComponents();
    ...
    private void initComponents() {
        ...
        String[] header = new String[]{"Desc","Total Price"};
        dtm.setColumnIdentifiers(header);

        for(OrderBundle bundle : this.bundles){
            String[] items = new String[2];
            items[0] = bundle.getAssigment().getBundle().getAds();
            items[1] = String.valueOf(bundle.getAssigment().getBundle().getTotalPrice());
            dtm.addRow(items);
        }
        cart_table.setModel(dtm);
        ...
        ads_title.setFont(new java.awt.Font("Lucida Grande", 1, 20)); // NOI18N
        ads_title.setText("My Cart");
    ...
    private void buy_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_buy_btnMouseClicked
        // TODO add your handling code here:
        int selected_row = this.cart_table.getSelectedRow();
        if (selected_row >= 0) {
            DataUtil.submitOrder(data, this.bundles.get(selected_row).getAssigment(), cp);
            this.bundles.remove(selected_row);
        }
        DefaultTableModel dtm = (DefaultTableModel) this.cart_table.getModel();
        dtm.setRowCount(0);
        for(OrderBundle bundle : this.bundles){
            String[] items = new String[2];
            items[0] = bundle.getAssigment().getBundle().getAds();
